...
element4.innerHTML = '<input type="file" multiple id="in3" accept=".tsv" >';
var fileInput3 = element4.firstChild;
                                                   
var fileSelect3 = document.getElementById("cur3");

fileInput3.addEventListener('change', function (evnt) 
  {
    var fileList = [];
    for (var i = 0,j = fileInput3.files.length; i<j; i++) 
      {
  	fileList.push(fileInput3.files[i]);
	...
	d3.tsv("Data/" +name, function(data) 
	  {    /* To get Data from external file we add cg.2.tsv and calls an 
	    ...
	    var height=280;
            ...
            if ($('#cleanbg svg').length === 6) 
	    {
        	$('#cleanbg svg:first').remove()
            }
	  
	  
	     var svg = d3.select("#cleanbg").append("svg")
	                 .attr("width", width)
                         .attr("height", height)
                         .append("g")
...
fileSelect3.addEventListener("click", function () 

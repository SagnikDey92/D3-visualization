...
element3.innerHTML = '<input type="file" multiple id="in2" accept=".tsv" >';
var fileInput2 = element3.firstChild;
                                                   
var fileSelect2 = document.getElementById("cur2");

fileInput2.addEventListener('change', function (evnt) 
  {
    var fileList = [];
    for (var i = 0,j = fileInput2.files.length; i<j; i++) 
      {
  	fileList.push(fileInput2.files[i]);
	...
	d3.tsv("Data/" +name, function(data) 
	  {    /* To get Data from external file we add cg.2.tsv and calls an 
	    ...
	    var height=280;
            ...
            if ($('#newbg svg').length === 6) 
	    {
        	$('#newbg svg:first').remove()
            }
	  
	  
	     var svg = d3.select("#newbg").append("svg")
	                 .attr("width", width)
                         .attr("height", height)
                         .append("g")
...
fileSelect2.addEventListener("click", function () 
